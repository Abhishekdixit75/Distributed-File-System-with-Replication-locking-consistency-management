...
def main():
	...
	filename_locked_map = {}    # helps in tracking whether a file is locked or not 
	filename_clients_map = defaultdict(list)  #stores the queue of the clients waiting for each file
	waiting_client = False
	client_timeout_map = {}  #tracks the number of times a client has requested a locked file 
	...
	while 1:
		...
		if "_1_:" in recv_msg:
			...
			if unlocked == True:
				...
				if client_id in client_timeout_map:		# check if the client has requested file before
					client_timeout_map[client_id] += 1	 # if not first time, increment timeout value of client	
					print("TIME: " + str(client_timeout_map[client_id]))
				else:
					client_timeout_map[client_id] = 0 # if first time, set timeout value to 0	
