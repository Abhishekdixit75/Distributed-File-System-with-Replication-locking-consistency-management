...
curr_path = os.getcwd()      # get path of current program (client.py)
...
def lock_unlock_file(client_socket, client_id, filename, lock_or_unlock):
    ...
    serverName = 'localhost'
    serverPort = 4040   # port of locking service
    client_socket.connect((serverName,serverPort))
...
def send_write(client_socket, fileserverIP_DS, fileserverPORT_DS, filename , RW, file_version_map, msg):
    ...
    # send the string requesting a write to the file server
...
def handle_write(filename, client_id, file_version_map):
    ...
    reply_DS = send_directory_service(client_socket, filename, 'a+', False)  # request the file info from directory service
    ...
    if reply_DS == "FILE_DOES_NOT_EXIST":
        ...
        if not read_cache:
            ...
            if reply_FS != "EMPTY_FILE":
                ...
                cache(filename_DS, reply_FS, "a+", client_id)  # update the cached file with the new version from the file server
