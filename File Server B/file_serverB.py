...
def read_write(filename, RW, text, file_version_map):
	if RW == "r":	# if read request
		if os.stat(os.path.join(os.getcwd(),(f"fileserverB\\{filename}"))).st_size != 0:
			file = open(os.path.join(os.getcwd(),(f"fileserverB\\{filename}")), RW)	
...
def main():
	while 1:
		...
		if (recv_msg != "") and ("CHECK_VERSION" not in recv_msg) and ("REPLICATE" not in recv_msg):
			...
			f = open(os.path.join( os.getcwd() , f"fileserverB\\{rep_filename}"), 'a+')
			f.write(rep_text + "\n")
			f.close()
